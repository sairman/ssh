...
echo "iptables -t nat -I POSTROUTING -s 10.8.0.0/24 -o venet0 -j MASQUERADE" > /etc/iptables8.conf
...
echo "iptables -t nat -I POSTROUTING -s 10.9.0.0/24 -o venet0 -j MASQUERADE" > /etc/iptables9.conf
...
sed -i "s|remote ip 55|remote $IP 55|" /etc/openvpn/clientconfig/client-tcp.ovpn
